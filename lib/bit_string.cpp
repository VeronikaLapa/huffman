...
void bit_string::add_bit(bool bit) {
    ...
    if (last_char_length == CHAR_SIZE || data.size() == 0) {
        data.push_back(b);
        last_char_length = 1;
    } else {
...
bool bit_string::get_bit(size_t i) {
    if (i >= size()) {
        ...
        return (data[char_ind] & (1 << char_pos));
    }
}
...
bit_string::bit_string(std::string str, int length) : data(str), last_char_length(length) {}
